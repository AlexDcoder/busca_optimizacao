...
def func(x1, x2):
    return np.exp(-((x1**2 + x2**2))) + (2 * np.exp(
        -(((x1 - 1.7)**2 + ((x2 - 1.7)**2)))))


def perturb(x, lower_bound, upper_bound, e=0.7):
...
x1_axis = np.linspace(-2, 4)
x2_axis = np.linspace(-2, 5)
...
algo = Algo(x1_axis, x2_axis, 10000, 100, 10)
hc_results = []
lrs_results = []
grs_results = []
...
ax.set_title(
    r'$f(x_1, x_2) = e^{-(x_1^2 + x_2^2)} + 2e^{-(x_1-1.7)^2-(x_2-1.7)^2}$',
    fontsize=12)
ax.set_xlabel(r'$x_1$')
ax.set_ylabel(r'$x_2$')
