...
def func(x1, x2):
    return -np.sin(x1) * (np.sin((x1**2)/np.pi)**(2 * 10)) - np.sin(x2) * (
        np.sin((2*x2**2)/np.pi)**(2 * 10))
...
x1_axis = np.linspace(0, np.pi)
x2_axis = np.linspace(0, np.pi)
...
ax.set_title(
    r'$f(x_1, x_2) = -\sin(x_1) \sin\left(\frac{x_1^2}{\pi}\right)^{20} - \sin(x_2) \sin\left(\frac{2x_2^2}{\pi}\right)^{20}$', fontsize=9)
ax.set_xlabel(r'$x_1$')
ax.set_ylabel(r'$x_2$')
